...
<div class="col-md-6">
    ...
    <div class="input-group">
        <div class="input-group-btn">
            <a class="btn btn-success" onclick="$('input[id=favicon]').click();">Browse</a>

        </div><!-- /btn-group -->

        <input type="text" name="SelectedfaviconName" class="form-control" id="SelectedfaviconName"
               value="{{$settings->favicon}}" readonly>

    </div>
    <div style="clear:both;"></div>
    <p class="help-block">File Extention must be jpg, jpeg, png. resolation(100*100)</p>
    <span id="error_favicon" class="has-error"></span>
    <script type="text/javascript">
        $('input[id=favicon]').change(function () {
            $('#SelectedfaviconName').val($(this).val());
        });
    </script>
</div>
<div class="clearfix"></div>
