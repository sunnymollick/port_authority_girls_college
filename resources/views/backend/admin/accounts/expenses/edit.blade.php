<form id='edit' action="" enctype="multipart/form-data" method="post" accept-charset="utf-8">
    ...
    <div class="form-group col-md-6 col-sm-12">
        ...
        <select name="expense_category_id" class="form-control filter" required>
            <option value="{{$expense->method}}" selected disabled>Select Expense Category</option>
            @foreach($expense_category as $category)
                <option value="{{$category->id}}"
                    {{ ( $category->id == $expense->expense_category_id) ? 'selected' : '' }} >{{$category->name}}</option>
            @endforeach
        </select>
    ...
    <div class="form-group col-md-4 col-sm-12">
        ...
        <select name="method" id="method" class="form-control" required>
            <option value="{{$expense->method}}" selected>{{$expense->method}}</option>
    ...
    <div class="form-group col-md-4 col-sm-12">
        ...
        <input type="text" class="form-control" id="expense_date" name="expense_date" value="{{$expense->expense_date}}"
