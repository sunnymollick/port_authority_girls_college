<form id='edit' action="" enctype="multipart/form-data" method="post" accept-charset="utf-8">
        ...
        <select id="bank_id" name="bank_id" class="form-control filter">
            ...
            @foreach($banks as $bank)
                <option value="{{$bank->id}}"
                    {{ ( $bankAccount->bank_id == $bank->id) ? 'selected' : '' }}>{{$bank->bank_name}}</option>
            @endforeach
        </select>
    ...
    <div class="form-group col-md-3 col-sm-12">
        ...
        <input type="text" class="form-control" id="contact_person" name="contact_person"
               value="{{ $bankAccount->contact_person }}"
               placeholder="">
        <span id="error_contact_person" class="has-error"></span>
    </div>
    <div class="form-group col-md-3 col-sm-12">
        <label for=""> Phone </label>
        <input type="text" class="form-control" id="phone" name="phone" value="{{ $bankAccount->phone }}"
               placeholder="">
    ...
    <div class="form-group col-md-3 col-sm-12">
        ...
        <input type="text" class="form-control" id="email" name="email" value="{{ $bankAccount->email }}"
               placeholder="">
    ...
    <div class="form-group col-md-12">
        ...
        <input type="radio" name="status" class="flat-green"
               value="1" {{ ( $bankAccount->status == 1 ) ? 'checked' : '' }} /> Active
        <input type="radio" name="status" class="flat-green"
               value="0" {{ ( $bankAccount->status == 0 ) ? 'checked' : '' }}/> In Active
    </div>
    <div class="clearfix"></div>
    <div class="form-group col-md-12">
